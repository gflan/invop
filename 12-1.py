...
def objetivo(problem):
    ...
    problem.variables.add(obj = my_obj, ub = my_ub)

def constraints(problem):

    #dureza maxima
    for mes in range(cant_meses):
        row_ind = [mes+tipo for tipo in range(cant_tipos)]
        row_val = dureza
        problem.linear_constraints.add(lin_expr=[cplex.SparsePair(ind=row_ind, val=row_val)], rhs=[6.0], senses=["L"])

    #dureza minima
    for mes in range(cant_meses):
        row_ind = [mes+tipo for tipo in range(cant_tipos)]
        row_val = dureza
        problem.linear_constraints.add(lin_expr=[cplex.SparsePair(ind=row_ind, val=row_val)], rhs=[3.0], senses=["G"])

    # 500 de storage el ultimo dia
